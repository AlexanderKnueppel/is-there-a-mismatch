...
public class FMStatistics {
	...
	public double increaseFeature(IFeatureModel fm) {
		return ((double)fm.getNumberOfFeatures() - model.getNumberOfFeatures())/(double)model.getNumberOfFeatures()*100.0;
	}
	
	public double increaseConstraints(IFeatureModel fm) {
		return ((double)fm.getConstraintCount() - model.getConstraintCount())/(double)model.getConstraintCount()*100.0;
	}
}
