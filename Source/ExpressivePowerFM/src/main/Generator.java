...
public class Generator {
	...
	/**
	 * The index equals the number of features of the models at this
	 * position.<br>
	...
	/**
	 * first index: The models with an equivalent hash value modulo the selected
	 * modulo value<br>
	 * second index: Differing models
	 */
	private ArrayList<LinkedList<FeatureModelCounter>> sortedModels = new ArrayList<LinkedList<FeatureModelCounter>>(
			MODULO);
							...
							: n == 3 ? BigInteger.valueOf(10)
									: n == 4 ? BigInteger.valueOf(126)
											: n == 5 ? BigInteger.valueOf(2570)
													: n == 6 ? BigInteger.valueOf(113544) : BigInteger.valueOf(-1));
				} else {
					counter = (n == 2 ? BigInteger.valueOf(4)
							: n == 3 ? BigInteger.valueOf(16)
									: n == 4 ? BigInteger.valueOf(150) : BigInteger.valueOf(-1));
				}
			}
	...
	/**
	 * Sets the feature names of all generated Models without constraints to
	 * F0,F1,F2...
	 * 
	 * @param i
	 */
				...
				for (int k = 0; k < i; k++) {
					...
					if (k != j) {
						...
						for (IFeatureModel m : newModelsWithConstraints) {
							...
							model.addConstraint(factory.createConstraint(model,
									new Implies(new Literal("F" + j), new Literal("F" + k))));
							...
							if (!COUNT_INVALID_MODELS) {
								if (analyser.isValid() && analyser.getDeadFeatures(1000000).isEmpty()
										&& !isRedundant(m, model)) {
							...
							model.addConstraint(factory.createConstraint(model,
									new Implies(new Literal("F" + j), new Not(new Literal("F" + k)))));
							if (!COUNT_INVALID_MODELS) {
								if (analyser.isValid() && analyser.getDeadFeatures(1000000).isEmpty()
										&& !isRedundant(m, model)) {
									newModelsWithConstraints2.add(model);
	 ...
	 *            - Number of features
	 * @param configs
	 *            - Set of Strings to permute
	 * @return result - Set of Strings including input Strings and permuted
	 *         Strings
	 */
